...
namespace kinem{
  ...
  inline double signed_delta_phi(double phi1, double phi2);

   //  calculate phi1-phi2 keeping value between 0 and pi
  inline double delta_theta(double theta1, double theta2);
...
inline
double kinem::delta_phi(double phi1, double phi2) //deltaphi for deltaR [0,Pi] not for ploting deltaPHI
{
...
inline
double kinem::delta_theta(double theta1, double theta2) //deltatheta for deltaR [0,Pi] not for ploting deltaTHETA
{
  double THETA=fabs(theta1-theta2);
  return (THETA<=PI)? THETA : kinem::TWOPI-THETA;
}
