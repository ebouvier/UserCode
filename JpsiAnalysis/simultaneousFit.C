  ...
  //---------------------------------------------------------------
{
  ...
  leg_par_2->AddEntry(gr_mean_gaus, TString::Format("#alpha = %2.2f + %2.4f#upointM_{t}",p0_ncat.getVal(),p1_ncat.getVal()), "l");
  ...
  leg_par_3->AddEntry(gr_mean_gaus, TString::Format("#gamma = %2.1f + %2.4f#upointM_{t}",p0_gamma_gam.getVal(),p1_gamma_gam.getVal()), "l");
  ...
  leg_par_4->AddEntry(gr_mean_gaus, TString::Format("#beta = %2.2f + %2.2f#upointM_{t}",p0_beta_gam.getVal(),p1_beta_gam.getVal()), "l");
  ...
  leg_par_5->AddEntry(gr_mean_gaus, TString::Format("#mu = %2.0f + %2.2f#upointM_{t}",p0_mu_gam.getVal(),p1_mu_gam.getVal()), "l");
  ...
  for (unsigned int ittbar = 0; ittbar <= nttbar; ittbar++) {
    ...
    if (ittbar > 0) {
      ...
      if (blind) {
        ...
        cms_myStyle(lumi, true);
        double y2_err = hist_err->GetBinContent(1.+mtop_res[1]/10.*hist_err->GetNbinsX());
        double y1_err = y2_err;
        if (fileData.Contains("Run")) 
          y1_err += 100.;
        else
          y1_err += 50.;
        TArrow ar_err(mtop_res[0], y1_err, mtop_res[0], y2_err, 0.025, "|>");
        ar_err.SetAngle(40);
