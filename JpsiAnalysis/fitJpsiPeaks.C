...
//---------------------------------------------------------------
double *cbPeak(TString inFile, TString outFile, TString channel, double lumi)
  //---------------------------------------------------------------
{
  ...
  double *mjpsi_res = new double[2];
  mjpsi_res[0] = mean_cball.getVal();
  mjpsi_res[1] = mean_cball.getError();
  return mjpsi_res;
}

//---------------------------------------------------------------
double *gPeak(TString inFile, TString outFile, TString channel, double lumi)
  //---------------------------------------------------------------
{
  using namespace RooFit;
  TLatex* channel_tex = new TLatex(0.22, 0.9, channel);
  ...
  double *mjpsi_res = new double[2];
  mjpsi_res[0] = mean_gaus.getVal();
  mjpsi_res[1] = mean_gaus.getError();
  return mjpsi_res;
}
...
{  
  if (date.Length() > 0 && version.Length() > 0 && fileData.Length() > 0 && decay.Length() > 0)  {
    ...
    TString outdir = indir+"JpsiPeaks/";
    TString outFileData = outdir;
    TString outFileMC = outdir;
    TString channel = " + Jets channel";
    TString fileMC = "";
    if (fileData.Contains("el", TString::kIgnoreCase)) {
      indir += "MyAnaEl/";
      fileData = indir+"ElectronHadASingleElectronBCD.root";
      fileMC = indir+"TTJets_MSDecays_JpsiFilter_172_5.root";
      outFileData += "DataFit_ElChannel";
      outFileMC += "MCFit_ElChannel";
    ...
    if (fileData.Contains("mu", TString::kIgnoreCase)) {
      ...
      fileMC = indir+"TTJets_MSDecays_JpsiFilter_172_5.root";
      outFileData += "DataFit_MuChannel";
      outFileMC += "MCFit_MuChannel";
      if (decay.Contains("semi", TString::kIgnoreCase))
        channel = "#mu"+channel;
      if (decay.Contains("di", TString::kIgnoreCase))
    ...
    if (fileData.Contains("all", TString::kIgnoreCase)) {
      ...
      fileMC = indir+"TTJets_MSDecays_JpsiFilter_172_5.root";
      outFileData += "DataFit_AllChannel";
      outFileMC += "MCFit_AllChannel";
      if (decay.Contains("semi", TString::kIgnoreCase))
