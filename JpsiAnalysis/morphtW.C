...
/*
 * morph tW trilepton mass histogram using 166.5 and 178.5
 */
...
int morphtW(TString date, TString version, TString channel){
  ...
  vector<TString> tW; tW.push_back("TToDilepton_tW-channel-DR_mass"); tW.push_back("TToThadWlep_tW-channel-DR_mass"); tW.push_back("TToTlepWhad_tW-channel-DR_mass"); tW.push_back("TBarToDilepton_tW-channel-DR_mass"); tW.push_back("TBarToThadWlep_tW-channel-DR_mass"); tW.push_back("TBarToTlepWhad_tW-channel-DR_mass");
  vector<float> ngen1; ngen1.push_back(1489880.); ngen1.push_back(497598.); ngen1.push_back(497610.); ngen1.push_back(1478200.); ngen1.push_back(497614.); ngen1.push_back(495229.);
  vector<float> ngen2; ngen1.push_back(1493428.); ngen2.push_back(489678.); ngen2.push_back(497165.); ngen2.push_back(1493389.); ngen2.push_back(497828.); ngen2.push_back(454540.);
  ...
  for (unsigned int itW = 0; itW < tW.size(); itW++) {
    cout << "\n" << tW[itW] << endl;

    TFile *f1 = TFile::Open(dir+tW[itW]+TString::Format("%.1f",mtop[0]).ReplaceAll(".","_")+".root");
    TH1F* h1 = (TH1F*) f1->Get("MTriLept-allPair");
    h1->Scale(1./ngen1[itW]);
    //h1->Rebin(5);
    double norm = h1->Integral();
    TFile *f2 = TFile::Open(dir+tW[itW]+TString::Format("%.1f",mtop[mtop.size()-1]).ReplaceAll(".","_")+".root");
    TH1F* h2 = (TH1F*) f2->Get("MTriLept-allPair");
    h2->Scale(1./ngen2[itW]);
    //h2->Rebin(5);

    for (unsigned int itop = 0; itop < mtop.size(); itop++) {
      cout << mtop[itop] << " GeV" << endl;

      TFile *outfi = TFile::Open(dir+tW[itW].ReplaceAll("-DR_","_")+TString::Format("%.1f",mtop[itop]).ReplaceAll(".","_")+".root","RECREATE");
      TH1F *outhisto = th1fmorph("MTriLept-allPair","MTriLept-allPair", h1, h2, mtop[0], mtop[mtop.size()-1], mtop[itop], norm);
      ...
      TH1F *ratio = (TH1F*)outhisto->Clone("ratio");
      //TH1F *ratio = th1fmorph("Ratio","Ratio", h1, h2, mtop[0], mtop[mtop.size()-1], mtop[itop], norm);
      ratio->Divide(h1);
      const int npoints = ratio->GetNbinsX();
      double x[npoints], ex[npoints], y[npoints], ey[npoints]; 
      for (int ipoint = 0; ipoint < npoints; ipoint++) {
        x[ipoint] = ratio->GetBinCenter(ipoint+1);
        ex[ipoint] = 0.;
        y[ipoint] = ratio->GetBinContent(ipoint+1);
        ey[ipoint] = ratio->GetBinError(ipoint+1);
      }
      TGraphErrors *w1 = new TGraphErrors(npoints,x,y,ex,ey);
      delete ratio;
      ...
      TTree *t1 = (TTree*) f1->Get("MTriLept");
      int ne1 = t1->GetEntries();
      float mass1; float weight1;
      t1->SetBranchAddress("mass", &mass1);
      t1->SetBranchAddress("weight", &weight1);
      for (int j = 0; j < ne1; j++) {
        t1->GetEntry(j);
        mass = mass1;
        weight = weight1*w1->Eval(mass1)/ngen1[itW];
        outtree->Fill();
